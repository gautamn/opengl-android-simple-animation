...
public class Sprite {
    ...
    public Sprite(final Context activityContext) {
        ...
        vertexBuffer = initFloatBuffer(square_1_coords);
        texCoordBuffer = initFloatBuffer(textureCoords);

        ByteBuffer dlb = ByteBuffer.allocateDirect(square_1_coords.length * 2);
        ...
        String fragmentShaderCode = ShaderUtil.readTextFileFromRawResource(activityContext, R.raw.texture_fragment_shader);
        ...
        mTextureDataHandle = TextureUtil.loadTexture(mActivityContext, R.drawable.sam);
    }
